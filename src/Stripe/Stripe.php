...
class Stripe extends PortAbstract implements PortInterface
{
    ...
    public function set($amount)
    {
       ...
       $this->currency = Config::get('gateway.stripe.currency') ?? 'USD';
    ...
    {
        if (!$this->callbackUrl) {
            $this->callbackUrl = Config::get('gateway.stripe.callback-url');
        }
    ...
    protected function sendPayRequest()
    {
        ...
        try {
            $response = Http::asForm()
                ->timeout(Config::get('gateway.stripe.timeout'))
                ->withToken(Config::get('gateway.stripe.client_secret'))
    ...
    public function verifyPayment(string $refId)
    {
        ...
        $response = Http::withToken(Config::get('gateway.stripe.client_secret'))
            ->timeout(Config::get('gateway.stripe.timeout'))
            ->get($url);
        ...
        $payment_response = Http::withToken(Config::get('gateway.stripe.client_secret'))
            ->timeout(Config::get('gateway.stripe.timeout'))
            ->get($payment_url);
